...
public class RouteService {
	...
	private static final List<String> BICYCLE_TIME_ROUTE_MODES = List.of("BIKE_ONLY", "SHORTEST", "ACCESSIBLE");
	...
	public RouteResponse findRoutes(RouteSearchRequest request) {
		...
		Place origin = resolvePlace(request.getStartPlaceNo(), request.getStartX(), request.getStartY(), "출발", "현재 위치");
		Place destination = resolvePlace(request.getEndPlaceNo(), request.getEndX(), request.getEndY(), "도착", "지정 도착지");

		List<Place> waypoints = resolveWaypoints(request.getWaypointPlaceNos(), transportType);
	...
	public List<OriginSearchResponse> searchOrigins(String query) {
		...
		return routeMapper.findPlacesByQuery(query.trim()).stream()
				.map(place -> OriginSearchResponse.builder().placeNo(place.getPlaceNo()).placeName(place.getPlaceName())
						.address(place.getAddress()).xAxis(place.getXAxis()).yAxis(place.getYAxis()).build())
				.toList();
	}

	private RouteResponse findCarRoutes(RouteSearchRequest request, Place origin, Place destination) {
		...
		JsonNode response = kakaoRouteClient.findCarRoute(origin.getXAxis(), origin.getYAxis(), destination.getXAxis(),
				destination.getYAxis(), priority, avoid);
	...
	private RouteResponse findPublicTransitRoutes(RouteSearchRequest request, Place origin, Place destination) {
		...
		JsonNode response = kakaoRouteClient.findPublicTransitRoutes(origin.getXAxis(), origin.getYAxis(),
				destination.getXAxis(), destination.getYAxis());
		...
		return buildResponse(TransportType.PUBLIC_TRANSIT, sortType.name(), origin, destination, routes);
	}

	private RouteResponse findBicycleRoutes(RouteSearchRequest request, Place origin, Place destination) {
		...
		if (option == RouteOption.MIN_TIME) {
			...
			JsonNode response = kakaoRouteClient.findBicycleRoute(origin.getXAxis(), origin.getYAxis(),
					destination.getXAxis(), destination.getYAxis(), routeMode);
			route = mapKakaoMapRoute(response, TransportType.BICYCLE.name());
		}

		return buildResponse(TransportType.BICYCLE, option.name(), origin, destination, List.of(route));
	}

	private RouteResponse findWalkingRoutes(RouteSearchRequest request, Place origin, Place destination,
		...
		JsonNode response = kakaoRouteClient.findWalkingRoute(origin.getXAxis(), origin.getYAxis(),
				destination.getXAxis(), destination.getYAxis(), routeMode, waypoints);
		RouteResultResponse route = mapKakaoMapRoute(response, TransportType.WALK.name());

		return buildResponse(TransportType.WALK, option.name(), origin, destination, waypoints, List.of(route));
	}

	private RouteResultResponse findFastestBicycleRoute(Place origin, Place destination) {
		...
		return candidates.stream().min(Comparator.comparingInt(RouteResultResponse::getTotalTime))
		...
		for (JsonNode route : response.path("routes")) {
			...
			for (JsonNode section : route.path("sections")) {
				...
				for (JsonNode guide : section.path("guides")) {
					steps.add(RouteStepResponse.builder().type(String.valueOf(guide.path("type").asInt()))
							.guidance(guide.path("guidance").asString()).distance(guide.path("distance").asInt())
							.time(guide.path("duration").asInt()).path(List.of(CoordinateResponse.builder()
									.xAxis(guide.path("x").asDouble()).yAxis(guide.path("y").asDouble()).build()))
							.build());
				}
			}

			results.add(RouteResultResponse.builder().routeType(TransportType.CAR.name())
					.totalDistance(summary.path("distance").asInt()).totalTime(summary.path("duration").asInt())
					.toll(summary.path("fare").path("toll").asInt()).steps(steps).path(path).build());
		}
	...
	private List<RouteResultResponse> mapAndSortTransitRoutes(JsonNode response, TransitType selectedType,
		...
		for (JsonNode route : response.path("routes")) {
			...
			for (int stepIndex = 0; stepIndex < routeSteps.size(); stepIndex++) {
				...
				steps.add(RouteStepResponse.builder().type(stepType)
						.guidance(stepProperties.path("guidance").asString())
						.distance(stepProperties.path("distance").asInt()).time(stepProperties.path("time").asInt())
						.stopNames(stopNames).vehicleNames(mapNames(stepProperties.path("vehicles"), "name"))
						// 대중교통 상세 안내용: 승하차·방향·정류장 수를 별도 필드로 정규화한다.
						.vehicleTypes(mapNames(stepProperties.path("vehicles"), "type"))
						.boardingPlace(firstName(stopNames)).alightingPlace(lastName(stopNames))
						.direction(transitDirection(stepType, stopNames))
						.stopCount(transitStopCount(stepType, stopNames))
						.transfer(isTransferWalkingStep(routeSteps, stepIndex, stepType)).path(stepPath).build());
			}

			mappedRoutes.add(RouteResultResponse.builder().routeType(routeType)
					.totalDistance(properties.path("totalDistance").asInt())
					.totalTime(properties.path("totalTime").asInt()).transfers(properties.path("transfers").asInt())
					.walkingDistance(walkingDistance).fare(properties.path("fare").path("value").asInt()).steps(steps)
					.path(path).build());
		}

		return mappedRoutes.stream()
				.filter(route -> selectedType == null || selectedType.name().equals(route.getRouteType()))
				.sorted(transitComparator(sortType)).toList();
	}
	...
	private RouteResultResponse mapKakaoMapRoute(JsonNode response, String routeType) {
		...
		for (JsonNode leg : route.path("legs")) {
			for (JsonNode step : leg.path("steps")) {
				...
				List<CoordinateResponse> stepPath = mapPointCoordinates(step.path("path").path("points"));
				path.addAll(stepPath);
				steps.add(RouteStepResponse.builder().guidance(stepProperties.path("guidance").asString())
						.distance(stepProperties.path("distance").asInt()).time(stepProperties.path("time").asInt())
						.path(stepPath).build());
			}
		}

		return RouteResultResponse.builder().routeType(routeType)
				.totalDistance(properties.path("totalDistance").asInt()).totalTime(properties.path("totalTime").asInt())
				.landingUrl(properties.path("landingUrl").asString()).steps(steps).path(path).build();
	}

	private RouteResponse buildResponse(TransportType transportType, String selectedOption, Place origin,
			Place destination, List<RouteResultResponse> routes) {
		return buildResponse(transportType, selectedOption, origin, destination, List.of(), routes);
	}

	private RouteResponse buildResponse(TransportType transportType, String selectedOption, Place origin,
			Place destination, List<Place> waypoints, List<RouteResultResponse> routes) {
		return RouteResponse.builder().transportType(transportType).selectedOption(selectedOption)
				.origin(toPlaceResponse(origin)).destination(toPlaceResponse(destination))
				.waypoints(waypoints.stream().map(this::toPlaceResponse).toList()).routes(routes).build();
	}

	private PlaceResponse toPlaceResponse(Place place) {
		return PlaceResponse.builder().placeNo(place.getPlaceNo()).placeName(place.getPlaceName())
				.address(place.getAddress()).xAxis(place.getXAxis()).yAxis(place.getYAxis()).build();
	}

	private List<Place> resolveWaypoints(List<Long> waypointPlaceNos, TransportType transportType) {
		...
		for (int index = 0; index < waypointPlaceNos.size(); index++) {
			...
			waypoints.add(resolvePlace(waypointPlaceNo, null, null, type, type));
		}
		return waypoints;
	}

	private Place resolvePlace(Long placeNo, Double xAxis, Double yAxis, String type, String directPlaceName) {
		boolean hasPlaceNo = placeNo != null;
		boolean hasAnyCoordinate = xAxis != null || yAxis != null;

		if (hasPlaceNo && hasAnyCoordinate) {
			throw new BadRequestException(type + " 장소 번호와 좌표 중 하나만 입력해야 합니다.");
		}
		...
		if (xAxis == null || yAxis == null) {
			throw new BadRequestException(type + " 장소의 경도와 위도를 모두 입력해야 합니다.");
		}
		if (xAxis < -180 || xAxis > 180 || yAxis < -90 || yAxis > 90) {
			throw new BadRequestException(type + " 장소의 좌표 범위가 올바르지 않습니다.");
		}

		return Place.builder().placeName(directPlaceName).xAxis(xAxis).yAxis(yAxis).build();
	}

	private void validateDistinctRoutePoints(Place origin, Place destination, List<Place> waypoints) {
		...
		for (int firstIndex = 0; firstIndex < routePoints.size(); firstIndex++) {
			for (int secondIndex = firstIndex + 1; secondIndex < routePoints.size(); secondIndex++) {
				if (samePlace(routePoints.get(firstIndex), routePoints.get(secondIndex))) {
					throw new BadRequestException("출발지, 도착지, 경유지는 서로 다른 장소여야 합니다.");
				}
			}
		}
	}
	...
	private boolean samePlace(Place first, Place second) {
		if (first.getPlaceNo() != null && second.getPlaceNo() != null
	...
	private List<CoordinateResponse> mapPointCoordinates(JsonNode points) {
		...
		for (JsonNode point : points) {
			if (point.isArray() && point.size() >= 2) {
				coordinates.add(CoordinateResponse.builder().xAxis(point.get(0).asDouble())
						.yAxis(point.get(1).asDouble()).build());
			}
		}
		return coordinates;
	}
	...
	private void addFlatCoordinates(List<CoordinateResponse> coordinates, JsonNode vertexes) {
		for (int index = 0; index + 1 < vertexes.size(); index += 2) {
			coordinates.add(CoordinateResponse.builder().xAxis(vertexes.get(index).asDouble())
					.yAxis(vertexes.get(index + 1).asDouble()).build());
		}
	}
	...
	private boolean isTransferWalkingStep(JsonNode routeSteps, int stepIndex, String stepType) {
		if (!"WALKING".equalsIgnoreCase(stepType) || stepIndex == 0 || stepIndex >= routeSteps.size() - 1) {
			return false;
		}

		String previousType = routeSteps.get(stepIndex - 1).path("properties").path("type").asString();
		String nextType = routeSteps.get(stepIndex + 1).path("properties").path("type").asString();
		return isTransitStep(previousType) && isTransitStep(nextType);
	}

	private boolean isTransitStep(String stepType) {
		return "BUS".equalsIgnoreCase(stepType) || "SUBWAY".equalsIgnoreCase(stepType);
	}
	...
	private Comparator<RouteResultResponse> transitComparator(TransitSortType sortType) {
		...
		case MIN_TRANSFER -> Comparator.comparingInt(RouteResultResponse::getTransfers)
				.thenComparingInt(RouteResultResponse::getTotalTime);
		case MIN_WALK -> Comparator.comparingInt(RouteResultResponse::getWalkingDistance)
				.thenComparingInt(RouteResultResponse::getTotalTime);
		};
	}
	...
	private void validateTransitStatus(JsonNode response) {
		...
		if ("STARTNODES_NULL".equals(status) || "ENDNODES_NULL".equals(status) || "NO_RESULTS".equals(status)) {
	...
	private void validateKakaoMapRouteStatus(JsonNode response) {
		...
		if ("START_LINK_NOT_FOUND".equals(status) || "END_LINK_NOT_FOUND".equals(status)
				|| "TOO_FAR_AWAY".equals(status) || "ROUTE_RESULT_NOT_FOUND".equals(status)) {
