...
public class BookmarkController {
	...
	public ResponseEntity<ApiResponse<BookmarkStatusResponse>> getBookmarkStatus(
		...
		Long memberNo = userDetails == null ? null : userDetails.getMemberNo();
		BookmarkStatusResponse data = bookmarkService.getBookmarkStatus(memberNo, placeNo);

		return ResponseEntity.status(200).body(ApiResponse.success("북마크 조회 성공", data));
	}
}
